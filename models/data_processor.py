...
flags.DEFINE_integer('dose_count', None, 'Filter only protocols having `dose_count` doses')
...
def process_protocols(path, from_file, to_file, dose_count):
    all_proto = pd.DataFrame(0, index=[],
                             columns=[i for i in range(0, dose_count*2 if dose_count else 40)])
    extension = 'csv'
    row_i = 0
    print("Processing protocols")
    for i in range(from_file, to_file):
        all_filenames = [i for i in glob.glob(path + str(i) + '/protocols/*.{}'.format(extension))]
        for f in tqdm(all_filenames, desc=str(i), ncols=100):
            temp = pd.read_csv(f, header=None)
            if dose_count and temp.shape[1] != dose_count:
                continue
            row = []
            if not dose_count:
...
def process_alive_counts(path, from_file, to_file):
    ...
    for i in range(from_file, to_file):
        all_filenames = [i for i in glob.glob(path + str(i) + '/aliveCount/*.{}'.format(extension))
                         if os.path.basename(i) in all_names]
        for f in tqdm(all_filenames, desc=str(i), ncols=100):
            temp = pd.read_csv(f)
            col_i = 0
...
def preprocess_data(dataset):
    ...
    #  dataset['y'].values.reshape(0, 1))
    return dataset


def create_dataset_and_save(protocols, alive_counts, path, from_file, to_file, dose_count):
    dataset = pd.concat([protocols, alive_counts], axis=1)
    dataset.columns = [str(i) for i in range(0, dose_count*2 if dose_count else 40)] + ['y']
...
def create_trainingset_and_save(dataset_path):
    ...
    for file in os.listdir(dataset_path):
        if file.startswith("dataset") and file.endswith(".csv"):
            dataset = pd.concat(
              [dataset, pd.read_csv(dataset_path + file).drop(['Unnamed: 0'], axis=1)],
              ignore_index=True)
    ...
    #    x_eval, y_eval, test_size=0.5, random_state=1)
...
def main(argv):
    ...
    print('Dose count: ' + str(FLAGS.dose_count))
    if FLAGS.concat_raw_data:
        ...
        protocols = process_protocols(FLAGS.protocol_path, FLAGS.from_file, FLAGS.to_file, FLAGS.dose_count)
        alive_counts = process_alive_counts(FLAGS.protocol_path, FLAGS.from_file, FLAGS.to_file)
        dataset = create_dataset_and_save(protocols, alive_counts, FLAGS.dataset_path,
                                          FLAGS.from_file, FLAGS.to_file, FLAGS.dose_count)
    else: 
